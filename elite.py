...
def defaultua():
    ua = ua_xiaomi
    try:ugent = open('ugent.json','w');ugent.write(ua)
...
def ugen():
    ...
    if pmu in[""]:
      ...
      input('%s╚══[ %sKembali %s]%s'%(K,P,K,P));menu()
        ...
        try:
          ugent = open('ugent.json','w').write(ua)
...
def ugen_hp():
    ...
    if pc in['']:
      jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P));menu()
    elif pc in ['1','01']:ugent = open('ugent.json','w').write(ua_xiaomi)
    elif pc in ['2','02']:ugent = open('ugent.json','w').write(ua_nokia)
    elif pc in ['3','03']:ugent = open('ugent.json','w').write(ua_asus)
    elif pc in ['4','04']:ugent = open('ugent.json','w').write(ua_huawei)
    elif pc in ['5','05']:ugent = open('ugent.json','w').write(ua_vivo)
    elif pc in ['6','06']:ugent = open('ugent.json','w').write(ua_oppo)
    elif pc in ['7','07']:ugent = open('ugent.json','w').write(ua_samsung)
    elif pc in ['8','08']:ugent = open('ugent.json','w').write(ua_windows)
    ...
    if "c_user" in ses.cookies:
      jalan(f"\r{P}[•]{I} Akun berhasil di login")
        ...
        for opt in range(len(ngew)):
            print(f"{B} | ")
            jalan(f"\n{U}[{B}{str(opt+1)}{U}]{P}--->{k}[{B}{ngew[opt]}{K}]")
